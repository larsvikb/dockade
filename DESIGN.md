...
those decisions and the backend enforces them. What stays here is the frontend itself,
when a lease stops counting, and what no guard in a browser can do.
...
- **Approval-UI follow-ups (reviewed and specified, not built).** From the same review
  ...
  pattern — are **now built**; see "Frontend mechanics" (the hold countdown) under the
  frontend section, and "A `+ persist` says what it will write" in
  `control-plane/DESIGN.md`. What became of the rest:
...
plane in `control-plane/DESIGN.md`. Still-planned additions to the tree:
