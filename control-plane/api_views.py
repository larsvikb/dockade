...
def api_audit(limit: int = 50, q: str | None = None, kind: str | None = None,
    ...
    ``audit.grouped`` explains the key, and control-plane/DESIGN.md, "The list groups;
    the record does not", the grouping. The columns are fewer than the table's: ``port``/
...
def api_saturation_ack(req: AckRequest) -> dict:
    ...
    the banner shows and no evidence. control-plane/DESIGN.md, "Dismissal is
    server-side, and it is a high-water mark", has the reasoning."""
    with holds._LOCK:
